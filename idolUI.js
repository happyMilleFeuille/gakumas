...
export function renderIdolList() {
    ...
    pssrArea.style.position = 'relative';
    pssrArea.innerHTML = `
        <button id="btn-idol-top-right" class="support-top-right-btn" aria-label="Idol Document">
    ...
    const idolTopRightBtn = pssrArea.querySelector('#btn-idol-top-right');
    if (idolTopRightBtn) {
        idolTopRightBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            const { openIdolPossessionModal } = await import('./idolPossessionModal.js');
